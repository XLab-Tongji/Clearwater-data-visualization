          ...
          function buildShow(jsonObj) {
            ...
            function build(nodes, links, lines) {
              ...
              function getEdgeBySourceTarget(source, target) {
                for (var i = 0; i < lines.length; i++) {
                  if (
                    lines[i].source.name == source &&
                    lines[i].target.name == target
                  ) {
                    return lines[i];
                  }
                  if (
                    lines[i].source.name == target &&
                    lines[i].target.name == source
                  ) {
                    return lines[i];
                  }
                }
                for (var i = 0; i < links.length; i++) {
                  if (
                    links[i].source.name == source &&
                    links[i].target.name == target
                  ) {
                    return links[i];
                  }
                }
              ...
              function show_paths_string() {
                ...
                for (var i = 0; i < paths_string.length; i++) {
                  ...
                  for (var j = 0; j < path_nodes.length - 1; j++) {
                    path_value =
                      path_value +
                      getEdgeBySourceTarget(path_nodes[j], path_nodes[j + 1])
                        .relation;
                  }
                  paths_objects[i] = {
                    path: _path_string,
                ...
                paths_objects.sort(function(a, b) {
                  ...
                  sub_paths_string_div
                    ...
                    .text(
                      "根因路径" +
                        (i + 1) +
                        "(权重分：" +
                        paths_objects[i].path_value +
                        ")" +
                        ":  " +
                        paths_objects[i].path
                    )
